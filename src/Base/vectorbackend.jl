...
function sortbyterm!(prop_cache::AbstractPropagationCache; lt=isless, by=identity, rev=false, order=Base.Forward, thread::Bool=true)
    ...
    AK.sortperm!(activeindices(prop_cache), activeterms(prop_cache); lt, by, rev, order, max_tasks=_maxtasks(thread))
...
function flag!(f::F, dst_flags, terms, coeffs; thread::Bool=true) where F<:Function
    ...
    AK.foreachindex(dst_flags; max_tasks=_maxtasks(thread)) do ii
...
flagstoindices!(dst_indices, flags; thread::Bool=true) = AK.accumulate!(+, dst_indices, flags; init=zero(eltype(dst_indices)), max_tasks=_maxtasks(thread))
...
function permuteviaindices!(dst_terms, dst_coeffs, src_terms, src_coeffs, indices; thread::Bool=true)
    ...
    AK.foreachindex(indices; max_tasks=_maxtasks(thread)) do ii
...
function filterviaflags!(flags, dst_indices, dst_terms, dst_coeffs, src_terms, src_coeffs; thread::Bool=true)
    ...
    AK.foreachindex(flags; max_tasks=_maxtasks(thread)) do ii
        if flags[ii]
...
function _copy!(dst_terms, dst_coeffs, src_terms, src_coeffs; thread::Bool=true)
    ...
    AK.foreachindex(src_terms; max_tasks=_maxtasks(thread)) do ii
